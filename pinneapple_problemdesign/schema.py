...
class PlanStep:
    ...
    pinneapple_modules: List[str] = field(default_factory=list)
...
@dataclass
class PinneappleSpec:
    """Concrete Pinneapple API objects derived from the elicited design spec.

    Produced by ``pinneapple_problemdesign.codegen.build_pinneapple_spec`` at
    ...
    Pinneapple stack at design time.

    Fields
    ------
    pde_kind : str
        Canonical PDE kind accepted by ``pinneapple_pinn.compile_problem`` and
        ``pinneapple_environment.ProblemSpec``.  ``"custom"`` when the PDE
    ...
    environment_kwargs : dict
        Keyword arguments for ``pinneapple_environment.ProblemSpec()``.
    model_name : str
    ...
    train_config_kwargs : dict
        Fields for ``pinneapple_train.TrainConfig``.
...
class DesignReport:
    ...
    gaps: List[Gap]
    plan: Plan
    pinneapple_spec: Optional["PinneappleSpec"] = None
