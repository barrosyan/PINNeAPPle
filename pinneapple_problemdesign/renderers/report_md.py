...
def render_markdown_report(report: DesignReport, warnings: List[str]) -> str:
    ...
    md = []
    md.append("# Pinneapple Design Report\n")
    ...
    md.append("\n## 6) Recommended approach (Pinneapple)\n")
    ...
    for i, step in enumerate(p.steps, start=1):
        ...
        if step.actions:
            ...
            for a in step.actions:
                md.append(f"- {a}")
        if step.pinneapple_modules:
            md.append("**Pinneapple modules involved:**")
            for m in step.pinneapple_modules:
                md.append(f"- `{m}`")
    ...
    ps = report.pinneapple_spec
    if ps is not None:
        md.append("\n## 9) Pinneapple API objects\n")
