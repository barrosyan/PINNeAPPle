"""Code generation: translate elicited ProblemSpec → concrete Pinneapple API objects.

Entry point
-----------
``build_pinneapple_spec(design_spec)`` reads the elicited
:class:`~pinneapple_problemdesign.schema.ProblemSpec` (business-level) and returns a
:class:`~pinneapple_problemdesign.schema.PinneappleSpec` containing:

* The identified PDE kind (via ``pinneapple_environment.capabilities.identify_pde``)
* Constructor kwargs for ``pinneapple_environment.ProblemSpec``
...
from .schema import ProblemSpec as DesignSpec, PinneappleSpec, uses_pinn_approach

# Sentinel for a physics parameter the user declared as "known" but for
# which no numeric value was elicited. Deliberately not a plausible float
# (like 1.0) so it fails loudly (e.g. float("REPLACE_ME") raises) instead of
# silently training against a wrong governing-equation parameter.
_UNRESOLVED_PARAM = "REPLACE_ME"
...
def _identify_pde(design_spec: DesignSpec) -> Dict[str, Any]:
    ...
    Falls back gracefully if pinneapple_environment is not installed.
    """
    try:
        from pinneapple_physics.pde_environment.capabilities import suggest_problem_spec
    except ImportError:
        return {
...
def _pinn_code(ps: PinneappleSpec, design_spec: DesignSpec) -> str:
    ...
    unresolved = sorted(k for k, v in ps.pde_params.items() if v == _UNRESOLVED_PARAM)
    unresolved_note = (
        f"        # TODO: fill in physical value(s) for: {', '.join(unresolved)}\n"
        if unresolved else ""
    )

    return textwrap.dedent(f"""\
        # ── Generated by Pinneapple DesignAgent ───────────────────────────────
...
def _fno_code(ps: PinneappleSpec, design_spec: DesignSpec) -> str:
    mk = dict(ps.model_kwargs)
    ...
    if is_forecasting:
        window_hint = textwrap.dedent("""\
            # from pinneapple_systems.time_series import WindowedDataset
    ...
    return textwrap.dedent(f"""\
        ...
        import pinneapple as pp
        from pinneapple_neural.trainer import Trainer, TrainConfig
        from pinneapple_neural.trainer.losses import SupervisedLoss
...
def build_pinneapple_spec(design_spec: DesignSpec) -> PinneappleSpec:
    """Translate an elicited :class:`~pinneapple_problemdesign.schema.ProblemSpec`
    into a :class:`~pinneapple_problemdesign.schema.PinneappleSpec`.

    Uses ``pinneapple_environment.capabilities.identify_pde`` for PDE detection
    and falls back gracefully when the environment package is unavailable.

    Parameters
    ----------
    design_spec:
        The business-level spec produced by :class:`~pinneapple_problemdesign.agent.DesignAgent`.
    ...
    if design_spec.physics.parameters_known:
        # Elicitation only captures the *names* of known params, never their
        # numeric value, so surface them as an explicit unresolved sentinel
        # rather than a silently-plausible-looking guessed float.
        for p in design_spec.physics.parameters_known:
            pde_params.setdefault(p, _UNRESOLVED_PARAM)
    ...
    # --- Code generation ---
    if uses_pinn_approach(design_spec):
