...
Uses exclusively existing PINNeAPPle modules — nothing built from scratch:
  ...
  pinneaple_timeseries.datasets.windowed  WindowedTimeSeriesDataset (sliding windows)
  pinneaple_timeseries.spec               TimeSeriesSpec
  pinneaple_train.trainer                 Trainer, TrainConfig  (training loop)
  pinneaple_train.losses                  CombinedLoss, SupervisedLoss
  pinneaple_train.normalizers             StandardScaler
  pinneaple_train.metrics                 MetricBundle + individual metric classes
  pinneaple_data.datasets                 load_dataset  (data loading)
  pinneaple_models / TSModelCatalog       model registry fallback
...
# ---------------------------------------------------------------------------
# Data loading  (pinneaple_data.datasets + file-path fallbacks)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Loss  (CombinedLoss from pinneaple_train.losses, with feature alignment)
# ---------------------------------------------------------------------------
...
class _TSTrainer:
    ...
    def __init__(self, model, loss_fn):
        from pinneaple_neural.trainer.trainer import Trainer
...
# ---------------------------------------------------------------------------
# Model factory  (pinneaple_timeseries.models + TSModelCatalog + ModelRegistry)
# ---------------------------------------------------------------------------

def _build_ts_model(name: str, n_features: int, horizon: int,
                    lookback: int) -> Any:
    """Build any time-series model via pinneaple_timeseries or ModelRegistry."""
    from pinneaple_systems.time_series.models import (
    ...
    try:
        from pinneaple_systems.time_series.registry import TSModelCatalog
...
class TimeSeriesBenchmarkSpec:
    ...
    def run(self) -> BenchmarkReport:
        ...
        from pinneaple_neural.trainer.normalizers import StandardScaler
        from pinneaple_systems.time_series.datasets.windowed import WindowedTimeSeriesDataset
        from pinneaple_systems.time_series.spec import TimeSeriesSpec
        ...
        # 2. Normalize — StandardScaler from pinneaple_train.normalizers
        ...
        for model_name in self.models:
            ...
            try:
                ...
                if is_neural:
                    from pinneaple_neural.trainer.trainer import TrainConfig
