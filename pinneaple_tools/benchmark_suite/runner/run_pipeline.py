...
def _load_config(config_path_or_dict: Union[str, Path, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        from pinneaple_tools.benchmark_suite.io.yamlx import load_yaml
...
def _step_geometry(geo_cfg: Dict[str, Any]) -> Any:
    ...
    try:
        from pinneaple_design.geometry.factory import build_geometry
...
def _step_solver(
    ...
    if backend == "openfoam":
        try:
            from pinneaple_simulation.numerical_solvers.openfoam_runner import run_openfoam
        ...
        try:
            from pinneaple_simulation.numerical_solvers.fenics_runner import run_fenics
        ...
        try:
            from pinneaple_simulation.numerical_solvers.problem_runner import run_builtin_solver
...
def _step_train_model(
    ...
    if do_compile:
        try:
            from pinneaple_neural.trainer import maybe_compile
    ...
    metrics_obj = None
    try:
        from pinneaple_neural.trainer.metrics_cfg import build_metrics_from_cfg
    ...
    if use_amp or grad_accum_steps > 1:
        try:
            from pinneaple_neural.trainer import AMPContext, GradAccumConfig, GradAccumTrainer
    ...
    # ------------------------------------------------------------------
    try:
        from pinneaple_neural.trainer import ThroughputMonitor
...
def _step_inference(
    ...
    """
    try:
        from pinneaple_neural.predictor import infer_on_grid_2d
    except ImportError:
    ...
    if len(spatial_coords) >= 2:
        ...
        try:
            from pinneaple_neural.predictor import infer_on_grid_1d
...
def _step_report(
    ...
    if plot_cfgs:
        try:
            from pinneaple_neural.predictor.visualize import render_visualizations
...
def _build_model_instance(
    ...
    # 1. PINNFactory
    try:
        from pinneaple_physics.pinn_solver.factory import PINNFactory
...
def _build_loss_fn(
    ...
    try:
        from pinneaple_neural.trainer.losses import build_loss
        # Attempt to compile PDE residuals
        physics_loss_fn = None
        try:
            from pinneaple_physics.pinn_solver.compiler import compile_problem
