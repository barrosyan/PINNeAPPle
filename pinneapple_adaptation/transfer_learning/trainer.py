...
* ``prepare()`` must be called before ``finetune()`` or
  ...
  :class:`~pinneapple_transfer.config.TransferConfig`.
* ``target_physics_fn`` follows the same contract as ``loss_fn`` in the
  main :class:`~pinneapple_train.trainer.Trainer`:
...
class TransferTrainer:
    ...
    :class:`~pinneapple_transfer.config.TransferConfig`, deep-copies the
    ...
    def finetune(
        ...
        target_physics_fn:
            ...
            follows the standard pinneapple convention (``x_col``, ``x_bc``,
