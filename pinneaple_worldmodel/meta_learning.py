...
Uses both Pinneaple-native meta-learning (``pinneaple_meta``) and built-in
...
:class:`MetaLearner`
    ...
    meta-gradient updates.  Falls back to ``pinneaple_meta.MAMLTrainer``
    and ``pinneaple_meta.ReptileTrainer`` when available.

Quick start::

    from pinneaple_worldmodel.meta_learning import MetaLearner, MetaConfig
    from pinneaple_worldmodel.dataset_factory import DatasetCatalog
...
class MetaConfig:
    ...
    algorithm : ``"maml"`` | ``"reptile"`` | ``"auto"``
        ``"auto"`` prefers pinneaple_meta if available, else reptile.
...
class MetaLearner:
    ...
    def meta_train(
        ...
        *,
        ...
        # Attempt to use pinneaple_meta
        if cfg.algorithm in ("auto", "reptile", "maml"):
            result = self._try_pinneaple_meta(catalog)
    ...
    # ------------------------------------------------------------------
    # pinneaple_meta integration
    # ------------------------------------------------------------------

    def _try_pinneaple_meta(
        self, catalog: DatasetCatalog
    ) -> Optional[PhysicsWorldModel]:
        """Attempt to use pinneaple_meta.MAMLTrainer or ReptileTrainer."""
        try:
            from pinneaple_adaptation.meta_learning import (  # type: ignore
        ...
        try:
            ...
            if cfg.verbose:
                print(f"[MetaLearner] pinneaple_meta.{TrainerCls.__name__} complete.")
            return meta_model
        except Exception as exc:
            log.debug("pinneaple_meta trainer failed: %s", exc)
            return None
