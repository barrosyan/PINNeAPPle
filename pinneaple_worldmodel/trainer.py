...
:class:`WorldModelTrainer` wraps :class:`~pinneaple_train.Trainer` and adds:
...
* **Physics consistency loss** — optional penalty ensuring the predicted field
  satisfies conservation laws, plugged in via ``pinneaple_validate``.
...
class WorldModelLoss:
    ...
    def __call__(
        ...
        """Compute loss dict compatible with :class:`~pinneaple_train.Trainer`.
...
class WorldModelTrainer:
    ...
    Integrates with ``pinneaple_train`` for optimizer / AMP / checkpointing;
