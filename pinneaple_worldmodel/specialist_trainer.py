"""Specialist model training using Pinneaple's full training stack.

:class:`SpecialistTrainer` trains one :class:`~.model.PhysicsWorldModel` per
physics domain (or per scenario) by leveraging:

* ``pinneaple_train`` — :class:`~pinneaple_train.Trainer` / :class:`~pinneaple_train.TwoPhaseTrainer`
  for optimizer, scheduling, gradient clipping, and AMP.
* ``pinneaple_validate`` — physics-informed validation: conservation-law
  checks, solver comparison, NaN/Inf monitoring.
* ``pinneaple_uq`` — Monte-Carlo Dropout or Aleatoric uncertainty estimation
  during validation to flag high-uncertainty predictions.
* ``pinneaple_transfer`` — :class:`~pinneaple_transfer.TransferTrainer` to
...
Quick start::
    ...
    from pinneaple_worldmodel.specialist_trainer import (
        SpecialistTrainer, SpecialistConfig,
    )
    from pinneaple_worldmodel.dataset_factory import PhysicsDatasetFactory, FactoryConfig
...
class SpecialistConfig:
    ...
    validate_physics : bool — run pinneaple_validate conservation checks.
...
class SpecialistTrainer:
    ...
    def _freeze_early_layers(self, model: PhysicsWorldModel) -> PhysicsWorldModel:
        ...
        try:
            from pinneaple_adaptation.transfer_learning import layer_lr_groups  # type: ignore
    ...
    def _run_physics_validation(
        ...
        """Run pinneaple_validate conservation-law checks."""
        try:
