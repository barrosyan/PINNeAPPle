...
5. **Uncertainty** — optional MC-Dropout or Aleatoric head (``pinneaple_uq``)
   gives calibrated prediction intervals alongside the mean field.

Integration with Pinneaple
--------------------------
* **pinneaple_models** — alternative backbone architectures (AFNO, SIREN,
  MeshGraphNet) can be hot-swapped via :class:`ModelRegistry`.
* **pinneaple_uq** — :class:`~pinneaple_uq.AleatoricHead` is attached when
  ``uncertainty="aleatoric"`` is requested.
* **pinneaple_transfer** — :func:`~pinneaple_transfer.layer_lr_groups`
  defines a layered learning-rate schedule for fine-tuning.
* **pinneaple_meta** — the model is designed to be the outer-loop model in
  MAML/Reptile; its LoRA adapters serve as the per-task fast weights.

Quick start::

    from pinneaple_worldmodel.mega_model import (
...
class PhysicsFoundationModel(nn.Module):
    ...
    def with_afno_backbone(
        ...
        *,
        ...
        Requires ``pinneaple_models.AFNO``.  Falls back silently to the
        standard FNO backbone if unavailable.
        """
        model = cls(config, n_fields=n_fields, grid_shape=grid_shape)
        try:
            from pinneaple_neural.architectures import AFNO  # type: ignore
            afno = AFNO(
