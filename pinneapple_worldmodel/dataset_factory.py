...
every data-production route available in Pinneapple:

* **Classical solvers** (``pinneapple_solvers``): FDM, LBM, FEM, SPH — the
  gold-standard for accuracy and speed on regular problems.
* **PINN data generation** (``pinneapple_pinn``): collocation-point sampling
  augmented by physics residuals; useful when classical solvers are unavailable.
* **Symbolic PDE integration** (``pinneapple_symbolic``): arbitrary PDEs defined
  symbolically generate both residual data and analytical solutions.
* **Collocation sampling** (``pinneapple_data``): adaptive sampling strategies
  (uniform, LHS, Sobol, importance-weighted) on arbitrary domains.
* **Environment auto-configuration** (``pinneapple_environment``): PDE
...
Typical usage::
    ...
    from pinneapple_worldmodel.dataset_factory import (
...
class FactoryConfig:
    ...
    sources : list of source names.
        ...
        ``"solver"`` uses pinneapple_solvers (FDM/LBM/FEM/SPH).
        ``"pinn"`` uses pinneapple_pinn.compile_problem for residual data.
        ``"symbolic"`` uses pinneapple_symbolic for analytical + residual data.
        ``"collocation"`` uses pinneapple_data.CollocationSampler directly.
    ...
    auto_discover_pde : bool — use pinneapple_environment to auto-identify PDE
...
class PhysicsDatasetFactory:
    ...
    def _generate_solver(self, scenario: PhysicsScenario) -> WorldModelDataset:
        """Generate trajectories using pinneapple_solvers or built-in FD solver."""
        cfg = self.config
    ...
    def _generate_pinn(self, scenario: PhysicsScenario) -> WorldModelDataset:
        """Generate collocation + PINN-residual data via pinneapple_pinn."""
        cfg = self.config
        pinn_available = False
        compile_problem = None

        try:
            from pinneapple_physics.pinn_solver import compile_problem as _compile_problem  # type: ignore
        ...
        if not pinn_available or compile_problem is None:
            if cfg.verbose:
                print(f"    [pinn] pinneapple_pinn not available; "
    ...
    def _generate_symbolic(self, scenario: PhysicsScenario) -> WorldModelDataset:
        """Generate data using pinneapple_symbolic for analytical solutions."""
        cfg = self.config
        symbolic_available = False

        try:
            from pinneapple_physics.symbolic_pde import SymbolicPDE  # type: ignore  # noqa: F401
        ...
        if not symbolic_available:
            if cfg.verbose:
                print(f"    [symbolic] pinneapple_symbolic not available; "
                      f"falling back to solver for {scenario.name}")
            return self._generate_solver(scenario)
    ...
    def _generate_collocation(self, scenario: PhysicsScenario) -> WorldModelDataset:
        ...
        if not colloc_available:
            if cfg.verbose:
                print(f"    [collocation] pinneapple_data not available; "
                      f"falling back to solver for {scenario.name}")
            return self._generate_solver(scenario)
    ...
    def _augment_with_discovered(
        ...
        """Use pinneapple_environment to discover additional related scenarios."""
        try:
            from pinneapple_physics.pde_environment.capabilities import list_pde_families  # type: ignore
