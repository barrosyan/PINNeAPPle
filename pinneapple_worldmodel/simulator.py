...
:class:`~pinneapple_worldmodel.scenario.PhysicsScenario`.  It tries each
available backend in priority order:

1. **pinneapple_solvers** (``fdm``, ``lbm``, ``fem``, ``sph``) — uses the
...
class PhysicsSimulator:
    ...
    def _resolve_solver(self):
        ...
        if kind != "builtin":
            fn = self._try_pinneapple_solver(kind, pde)
            if fn is not None:
                return fn
            if self.verbose:
                print(f"  [WorldModel] pinneapple_solvers.{kind} unavailable, "
                      f"falling back to builtin for '{pde}'.")

        return self._builtin_solver(pde)

    def _try_pinneapple_solver(self, kind: str, pde: str):
        """Try to build a solver using pinneapple_solvers. Returns None on failure."""
        try:
            if kind == "fdm":
    ...
    def _make_fdm_solver(self, pde: str):
        ...
        from pinneapple_simulation.numerical_solvers.fdm import FDMSolver  # type: ignore
    ...
    def _make_lbm_solver(self, pde: str):
        ...
        from pinneapple_simulation.numerical_solvers.lbm import LBMSolver  # type: ignore
    ...
    def _make_fem_solver(self, pde: str):
        ...
        from pinneapple_simulation.numerical_solvers.fem import FEMSolver  # type: ignore
