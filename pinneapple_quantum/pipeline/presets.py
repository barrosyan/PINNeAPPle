...
from pinneapple_quantum.training.trainer import QTrainer, QTrainerConfig
from pinneapple_quantum.data.quantum_dataset import (
...
def solve(
    ...
    **kwargs: Any,
    ...
    ----------
    problem : str
        Must be ``"quantum"`` (other problems route to the main ``pinneapple`` solve).
    equation : str
    ...
    **kwargs
    ...
    Examples
    --------
    >>> from pinneapple_quantum import solve
    >>> result = solve(
    ...     problem="quantum",
    ...
    if problem != "quantum":
        raise ValueError(
            f"pinneapple_quantum.solve only handles problem='quantum'. "
            f"Got {problem!r}. Use pinneapple.solve for classical problems."
        )
