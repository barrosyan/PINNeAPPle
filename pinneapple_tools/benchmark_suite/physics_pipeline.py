...
Uses:
  - pinneapple_environment.presets  (get_preset, list_presets → ProblemSpec)
  - pinneapple_pinn                 (compile_problem, LossWeights → loss_fn)
  - pinneapple_models               (ModelRegistry.build → any registered model)
  - pinneapple_train.metrics        (MSE, MAE, RMSE, R2, RelL2, MaxError)
  - pinneapple_geom                 (GeometrySpec, MeshData → mesh-based sampling)
  - pinneapple_solvers              (SolverCatalog → numerical reference data)

Usage
-----
    from pinneapple_tools.benchmark_suite import PhysicsPipeline
...
def _compute_metrics(pred: np.ndarray, true: np.ndarray,
    ...
    """Compute requested metrics using MetricBundle from pinneapple_train.metrics."""
...
# -----------------------------------------------------------------------------
# Collocation sampling  (pinneapple_data.collocation.CollocationSampler)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Built-in fallback problems  (used when no pinneapple_environment preset found)
# -----------------------------------------------------------------------------
...
def _generate_synthetic_data(
    ...
    Falls back to pinneapple_data.datasets if the solver is unavailable.
    """
    # ── Try pinneapple_solvers first ──────────────────────────────────────────
    try:
        from pinneapple_simulation.numerical_solvers import SolverCatalog
        cat = SolverCatalog()
    ...
    # ── Fallback: pre-computed dataset from pinneapple_data ───────────────────
...
def _physics_residual(prob: _ProblemDef, model: nn.Module,
    ...
    from pinneapple_physics.pinn_solver.compiler.autograd_ops import grad as _ag_grad
    u = _pred(model, pts)
    ...
    raise ValueError(f"No built-in residual for '{pid}'. "
                     "Use a pinneapple_environment preset instead.")
...
def _batch_from_geometry(geometry: Any, fields: List[str],
    ...
    Accepts:
      ...
      - pinneapple_geom.GeometrySpec
      - pinneapple_geom.MeshData
      - pinneapple_data STLMesh
      - any object with .sample_collocation() (returns {"x_col", "x_bc", ...})
    """
    rng = np.random.default_rng(seed)

    # ── pinneapple_geom.GeometrySpec → build MeshData ─────────────────────────
    try:
        from pinneapple_design.geometry import GeometrySpec, MeshData
        if isinstance(geometry, GeometrySpec):
            from pinneapple_design.geometry.mesh.mesher_2d import mesh_sdf_2d
    ...
    # ── STL file path ─────────────────────────────────────────────────────────
    try:
        from pinneapple_data.stl_import import load_stl, STLMesh
...
def _make_compiled_loss(spec: Any,
    ...
    """Return compile_problem loss for a ProblemSpec (pinneapple_pinn)."""
    from pinneapple_physics.pinn_solver import compile_problem, LossWeights
...
def _build_model(name: str, in_dim: int, out_dim: int,
    ...
    from pinneapple_neural.architectures import ModelRegistry, SIREN, ModifiedMLP, HashGridMLP
    from pinneapple_neural.architectures.pinns import VanillaPINN, PINNCatalog
...
class PhysicsPipeline:
    ...
    problem : str or ProblemSpec
        ...
        - Name of a pinneapple_environment preset (e.g. "burgers_1d_default").
        - Built-in alias (e.g. "burgers_1d", "heat_1d", "kovasznay_ns").
        - A ProblemSpec / custom object from pinneapple_environment.
        Call ``PhysicsPipeline.list_problems()`` to see all options.
    geometry : GeometrySpec | MeshData | str | None
        Step 2 — geometry & mesh:
        - ``pinneapple_geom.GeometrySpec`` → mesh auto-generated via SDF.
        - ``pinneapple_geom.MeshData`` → used directly for sampling.
        - str path to an STL file.
    ...
    def list_problems(cls) -> Dict[str, str]:
        ...
        try:
            ...
            for pid in list_presets():
                result.setdefault(pid, "(pinneapple_environment preset)")
        except Exception:
            pass
        return result
    ...
    def _resolve_problem(self) -> Tuple[Any, str]:
        ...
        if isinstance(prob, str):
            ...
            # 1. Try pinneapple_environment preset registry
            try:
            ...
            raise ValueError(
                ...
                "Use a pinneapple_environment preset name or one of: "
    ...
    def run(self) -> BenchmarkReport:
        ...
        if self.geometry is not None:
            ...
            print("    Loss     : compiled via pinneapple_pinn (geometry domain)")
