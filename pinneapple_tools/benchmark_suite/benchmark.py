...
Usage
-----
>>> from pinneapple_tools.benchmark_suite.benchmark import PINNArenaBenchmark
...
class _BuiltinProblemTask(BenchmarkTaskBase):
    ...
    def sample_boundary(self, n: int, seed: int) -> Tuple[np.ndarray, np.ndarray]:
        from pinneapple_tools.benchmark_suite.physics_pipeline import _make_bc_ic_pts
    ...
    def pde_residual(self, model: nn.Module, X: torch.Tensor) -> torch.Tensor:
        from pinneapple_tools.benchmark_suite.physics_pipeline import _physics_residual
        res = _physics_residual(self._p, model, X)
        return (res ** 2).mean()

    def eval_grid(self, n: int) -> Tuple[np.ndarray, np.ndarray]:
        from pinneapple_tools.benchmark_suite.physics_pipeline import (
...
class _PresetProblemTask(BenchmarkTaskBase):
    """Adapt a pinneapple_environment ProblemSpec to BenchmarkTaskBase."""
    ...
    def eval_grid(self, n: int) -> Tuple[np.ndarray, np.ndarray]:
        # Try pinneapple_data reference first
        try:
            from pinneapple_tools.benchmark_suite.physics_pipeline import _load_reference, _make_eval_grid
...
class PINNArenaBenchmark:
    ...
    def default(
        ...
        Problems are loaded from ``pinneapple_environment`` presets + built-in
        tasks. Models are loaded from ``pinneapple_models`` (ModelRegistry,
        ...
        try:
            ...
            from pinneapple_tools.benchmark_suite.tasks.poisson_2d import Poisson2DTask
            from pinneapple_tools.benchmark_suite.tasks.heat_1d import Heat1DTask
            from pinneapple_tools.benchmark_suite.tasks.wave_1d import Wave1DTask
            from pinneapple_tools.benchmark_suite.tasks.allen_cahn_1d import AllenCahn1DTask
            from pinneapple_tools.benchmark_suite.tasks.navier_stokes_tgv_2d import NavierStokesTGV2DTask
            all_tasks.update({
        ...
        try:
            from pinneapple_tools.benchmark_suite.physics_pipeline import _BUILTIN_PROBLEMS
        ...
        # 3. All pinneapple_environment presets
        try:
            from pinneapple_physics.pde_environment.presets.registry import list_presets, get_preset
