...
"""Arena backend that trains any model registered in ``pinneapple_models``.
...
This backend is meant to be the "multi-model" workhorse for Arena:
  ...
  - trains with pinneapple_train.Trainer (optionally DDP)
...
from pinneapple_tools.benchmark_suite.registry import register_backend
...
from pinneapple_neural.architectures.adapters import select_adapter
...
@register_backend
class PinneappleModelsBackend:
    """Train any model from ModelRegistry."""

    name = "pinneapple_models"
    ...
    def train(self, bundle: BundleData, run_cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not model_name:
            raise ValueError("run_cfg.model.name is required for pinneapple_models backend.")
