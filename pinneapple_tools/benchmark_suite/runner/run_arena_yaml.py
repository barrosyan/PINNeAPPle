...
- Any problem preset registered in ``pinneapple_environment``
- Multiple models per experiment (comparison benchmarks)
- Configurable metrics from ``pinneapple_train``
- Inference and visualization via ``pinneapple_inference``

YAML schema
-----------
See ``examples/pinneapple_arena/configs/experiment_burgers_1d.yaml`` for a
full annotated example.

Usage
-----
python -m pinneapple_arena.runner.run_arena_yaml \\
    --config examples/pinneapple_arena/configs/experiment_burgers_1d.yaml \\
...
def _build_dataset(
    ...
    if data_source == "bundle":
        ...
        from pinneapple_tools.benchmark_suite.pipeline.dataset_builder import build_from_bundle
        ...
        from pinneapple_tools.benchmark_suite.pipeline.dataset_builder import build_from_real_data
...
def main() -> None:
    import argparse
    ap = argparse.ArgumentParser(description="Run a pinneapple Arena experiment from YAML")
