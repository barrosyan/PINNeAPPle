...
Orchestrates the complete workflow:
  1. Load or generate geometry (pinneapple_geom)
...
def _load_config(config_path_or_dict: Union[str, Path, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        from pinneapple_tools.benchmark_suite.io.yamlx import load_yaml
...
def _step_geometry(geo_cfg: Dict[str, Any]) -> Any:
    ...
    # Try pinneapple_geom first
    try:
        from pinneapple_design.geometry.factory import build_geometry
        geometry = build_geometry(geo_type, **params)
        log.info("Geometry built via pinneapple_geom in %.2fs", time.perf_counter() - t0)
        return geometry
    except ImportError:
        log.debug("pinneapple_geom not available — trying fallback.")
    except Exception as exc:
        log.warning("pinneapple_geom build_geometry failed: %s", exc)
...
def _step_solver(
    ...
    if backend == "openfoam":
        try:
            from pinneapple_simulation.numerical_solvers.openfoam_runner import run_openfoam
            ...
            log.warning("pinneapple_solvers.openfoam_runner not found — returning empty solver data.")
        except Exception as exc:
            log.error("OpenFOAM solver failed: %s", exc, exc_info=True)

    elif backend == "fenics":
        try:
            from pinneapple_simulation.numerical_solvers.fenics_runner import run_fenics
            ...
            log.warning("pinneapple_solvers.fenics_runner not found — returning empty solver data.")
        except Exception as exc:
            log.error("FEniCS solver failed: %s", exc, exc_info=True)

    else:
        # builtin — delegate to pinneapple_solvers.problem_runner if available
        try:
            from pinneapple_simulation.numerical_solvers.problem_runner import run_builtin_solver
            ...
            log.debug("pinneapple_solvers not available — builtin solver skipped.")
        except Exception as exc:
...
def _step_dataset(
    ...
    try:
        ...
        log.debug("pinneapple_solvers not available — building minimal dataset.")
    except Exception as exc:
...
def _step_train_model(
    ...
    if do_compile:
        try:
            from pinneapple_neural.trainer import maybe_compile
    ...
    metrics_obj = None
    try:
        from pinneapple_neural.trainer.metrics_cfg import build_metrics_from_cfg
    ...
    if use_amp or grad_accum_steps > 1:
        try:
            from pinneapple_neural.trainer import AMPContext, GradAccumConfig, GradAccumTrainer
    ...
    # ------------------------------------------------------------------
    try:
        from pinneapple_neural.trainer import ThroughputMonitor
...
def _step_inference(
    ...
    try:
        ...
        log.debug("pinneapple_inference not available — skipping inference.")
        return None
    ...
    if len(spatial_coords) >= 2:
        ...
        try:
            from pinneapple_neural.predictor import infer_on_grid_1d
...
def _step_report(
    ...
    # ------------------------------------------------------------------
    # Visualizations via pinneapple_inference
    # ------------------------------------------------------------------
    if plot_cfgs:
        try:
            from pinneapple_neural.predictor.visualize import render_visualizations
            ...
            log.debug("pinneapple_inference.visualize not available — skipping plots.")
        except Exception as exc:
...
def _build_model_instance(
    ...
    # 1. PINNFactory
    try:
        from pinneapple_physics.pinn_solver.factory import PINNFactory
        ...
        log.debug("pinneapple_pinn.factory not available.")
    except Exception as exc:
    ...
    try:
        ...
        log.debug("pinneapple_models.registry not available.")
    except Exception as exc:
    ...
    *,
    ...
    if problem_cfg:
        try:
            ...
            log.debug("pinneapple_environment not available — problem_spec will be None.")
        except Exception as exc:
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="pinneapple end-to-end pipeline: geometry → solver → dataset → train → predict → report"
    )
    ap.add_argument(
        "--config",
        type=str,
