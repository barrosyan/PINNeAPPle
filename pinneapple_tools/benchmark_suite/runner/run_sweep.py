...
def run_sweep(
    *,
    ...
    for m in models:
        ...
        cfg["backend"].setdefault("name", "pinneapple_models")
        cfg["model"]["name"] = name
    ...
    if parallelism == "sequential" or n_gpu_total == 0:
        for job in jobs:
            ...
            cmd = [python_exe, "-m", "pinneapple_arena.runner.run_benchmark", "--bundle", bundle_root, "--run", str(run_dir)]
            env = os.environ.copy()
            env["PINNEAPPLE_ARENA_CFG"] = str(run_dir / "run_cfg.json")
    ...
    def launch(job: SweepJob, gpu_ids: List[int]):
        ...
        if ddp_per_model and len(gpu_ids) > 1:
            cmd = [
                ...
                "-m",
                "pinneapple_arena.runner.run_benchmark",
                "--bundle",
                bundle_root,
                "--run",
                str(run_dir),
