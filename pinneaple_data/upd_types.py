...
pinneaple_pinn) lives in pinneaple_pinn.io.upd_dataset and imports from here.
"""
...
def seconds_since(t: np.ndarray, t0: Optional[np.datetime64] = None) -> np.ndarray:
    ...
    *t0* defaults to ``t.min()`` when ``None``.  The result is a ``float64``
    ...
    This utility belongs in ``pinneaple_data`` because it is a general-purpose
...
class Batch:
    ...
    Compatible with the loss_fn produced by ``pinneaple_pinn.compiler``:
