"""FEniCS integration — workflow layer on top of pinneaple_solvers.fenics_bridge.

The low-level bridge (pinneaple_solvers.fenics_bridge.FEniCSBridge) implements
...
def solve_and_package(config: FEniCSConfig):
    ...
    """
    try:
        from pinneaple_simulation.numerical_solvers.fenics_bridge import FEniCSBridge
    except ImportError:
        raise ImportError(
            "pinneaple_solvers.fenics_bridge requires FEniCS. "
            "Install dolfinx or legacy FEniCS, then reinstall pinneaple_solvers."
        )
