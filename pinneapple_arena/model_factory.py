...
Uses ModelRegistry.build() (+ instantiate normalization) from pinneapple_neural
...
def _build_kwargs(net_cfg: NetworkConfig, in_dim: int, out_dim: int,
    ...
    instantiate() from pinneapple_neural will filter and normalize these,
...
def build_model(cfg: ModelConfig, in_dim: int = 2, out_dim: int = 1,
    ...
    """Instantiate any pinneapple model from a ModelConfig.
...
def _build_noether(model_type: str, net_cfg: NetworkConfig, in_dim: int, out_dim: int
    ...
    try:
        from pinneapple_neural.architectures.neural_operators.noether_bridge import NOETHER_REGISTRY
        cls = NOETHER_REGISTRY[model_type]
        return cls(**net_cfg.extra)
    except Exception as e:
        raise ImportError(
            f"Noether model '{model_type}' not available. "
            "Install with: pip install pinneapple[noether]"
        ) from e
