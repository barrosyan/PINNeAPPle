...
# --- PINNeAPPle ---
from pinneapple_physics.pinn_solver.factory.pinn_factory import PINNProblemSpec, PINNFactory, PINN
from pinneapple_neural.architectures.pinns.vanilla import VanillaPINN
from pinneapple_neural.architectures.pinns.vpinn import VPINN
from pinneapple_neural.architectures.pinns.pinnsformer import PINNsFormer
from pinneapple_neural.architectures.pinns.pielm import PIELM
...
class PIELMGradWrapper(nn.Module):
    ...
    which blocks gradients (see pinneapple_models/pinns/pielm.py).
    """
...
def main():
    ...
    # Save JSON
    out_json = os.path.join(cfg.out_dir, "pinneapple_pure_benchmark_results.json")
    ...
    if cfg.save_csv and len(results) > 0:
        out_csv = os.path.join(cfg.out_dir, "pinneapple_pure_benchmark_results.csv")
