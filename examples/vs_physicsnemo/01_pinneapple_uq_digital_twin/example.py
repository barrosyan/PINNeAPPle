...
# UQ
try:
    from pinneapple_analysis.uncertainty import MCDropoutWrapper, MCDropoutConfig, CalibrationMetrics, UQResult
    UQ_AVAILABLE = True
except ImportError:
    UQ_AVAILABLE = False
    print("[WARN] pinneapple_uq not found — UQ section will be skipped")
...
try:
    ...
    from pinneapple_systems.digital_twin.monitoring.anomaly import ThresholdDetector, ZScoreDetector, AnomalyMonitor
    DT_AVAILABLE = True
except ImportError:
    DT_AVAILABLE = False
    print("[WARN] pinneapple_digital_twin not found — DT section will be skipped")
...
def demo_uq(model: HeatPINN, device="cpu"):
    ...
    """
    if not UQ_AVAILABLE:
        print("[SKIP] pinneapple_uq not available")
        return None
...
def demo_digital_twin(model: HeatPINN, device="cpu"):
    ...
    """
    if not DT_AVAILABLE:
        print("[SKIP] pinneapple_digital_twin not available")
        return
