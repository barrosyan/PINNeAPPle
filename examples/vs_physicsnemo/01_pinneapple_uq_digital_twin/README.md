...
|---|---|---|
| Monte Carlo Dropout UQ | ✅ `pinneapple_uq.MCDropoutWrapper` | ❌ |
| Conformal Prediction (intervalos calibrados) | ✅ `pinneapple_uq.ConformalPredictor` | ❌ |
| Expected Calibration Error (ECE) | ✅ `pinneapple_uq.CalibrationMetrics` | ❌ |
| Digital twin com streams de sensores | ✅ `pinneapple_digital_twin.MockStream` | ❌ |
| Integração MQTT / Kafka / REST | ✅ `pinneapple_digital_twin.io.stream` | ❌ |
| Assimilação EKF / EnKF | ✅ `pinneapple_digital_twin.assimilation` | ❌ |
| Detecção de anomalias em tempo real | ✅ `pinneapple_digital_twin.monitoring` | ❌ |
...
from pinneapple_uq import MCDropoutWrapper, MCDropoutConfig
...
```bash
cd examples/vs_physicsnemo/01_pinneapple_uq_digital_twin
python example.py
```
