"""Geometry loading and attachment utilities for PhysicalSample via pinneapple_geom."""
...
#  - dict spec {"kind": "...", ...}
#  - already-built GeometryAsset (from pinneapple_geom)
...
class GeometryLoadOptions:
    ...
    Note: operations such as remeshing/decimation are expected to live in pinneapple_geom.ops.
    """
    # transforms
...
def _default_spec_from_path(p: Union[str, Path]) -> Dict[str, Any]:
    ...
    Dict[str, Any]
        ...
        pinneapple_geom's registry builder.
    """
    p = Path(p)
...
def load_geometry_asset(
    ...
    *,
    ...
    """
    Load or construct a GeometryAsset via pinneapple_geom.
    ...
    -------
    Any
        GeometryAsset instance as defined by pinneapple_geom.
    ...
    ImportError
        If pinneapple_geom is not available.
    """
    """
    Loads/creates a GeometryAsset using pinneapple_geom.

    Returns:
      GeometryAsset (opaque here; defined in pinneapple_geom)
    ...
    # We avoid importing pinneapple_geom types explicitly (keeps adapters light).
    ...
    try:
        from pinneapple_design.geometry.core.registry import build_geometry_asset  # type: ignore
    except Exception as e:
        raise ImportError(
            "pinneapple_geom is required to load geometry assets. "
            "Install geometry extras or add pinneapple_geom module."
        ) from e
