...
def has_pinnego_physical_sample() -> bool:
    """Check whether `pinneapple_data.physical_sample.PhysicalSample` is importable.

    Returns:
        True if `PhysicalSample` can be imported from `pinneapple_data`,
        otherwise False.
    """
    try:
        from pinneapple_data.physical_sample import PhysicalSample  # noqa: F401
        return True
    except Exception:
        return False
...
def to_physical_sample(
    ...
    *,
    ...
    Returns:
      - pinneapple_data.physical_sample.PhysicalSample if available
      - else returns a minimal dataclass fallback (SynthPhysicalSample)

    Args:
        sample_like: Input object to convert. Supported forms include:
            - An existing `pinneapple_data.physical_sample.PhysicalSample` instance
              (returned as-is).
