...
Single source of truth — pinneapple_train.splits re-exports from here.
"""
...
def _counts(n: int, train: float, val: float) -> tuple[int, int, int]:
    """Floor train and val; remainder goes to test. Guarantees n_train+n_val+n_test==n."""
    if n <= 0:
        return 0, 0, 0
...
def split_indices(
    ...
    *,
    ...
    if spec.method == "group":
        ...
        test_idx = idx[np.isin(group_ids_arr, list(set(uniq[g_train_n + g_val_n:])))]
...
__all__ = ["SplitSpec", "split_indices", "_counts"]
