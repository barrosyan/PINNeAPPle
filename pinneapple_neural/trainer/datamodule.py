...
from pinneapple_data.splits import SplitSpec, split_indices
from .preprocess import PreprocessPipeline

try:
    from pinneapple_data.collate import collate_pinn_batches as _collate_pinn
except Exception:
    _collate_pinn = None
...
class FnAdapter(ItemAdapter):
    """
    Convenience adapter for Pinneapple PhysicalSample-like objects (or any object).
...
class DataModule:
    ...
    def _collate(self, batch):
        """Collate list[dict] → dict; delegates to pinneapple_data.collate when available."""
        items = []
        for b in batch:
