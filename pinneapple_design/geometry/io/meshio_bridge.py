...
try:
    # Optional: only needed when packaging into pinneapple_data structures
...
def meshio_to_upd(path: str):
    ...
    if PhysicalSample is None:
        raise RuntimeError(
            "pinneapple_data is not available; cannot build PhysicalSample. "
            "Install/enable pinneapple_data or use load_meshio/save_meshio instead."
        )
