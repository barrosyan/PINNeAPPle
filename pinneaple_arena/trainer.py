...
Handles four paradigms:
  1. PINN          — physics residuals (own autograd OR pinneaple_physics compiled losses)
  2. Supervised    — MSE on (input, target) pairs
  3. Graph/GNN     — supervised on GraphBatch objects
  4. Inverse       — pinneaple_analysis.InverseProblemSolver

Post-training:
  - UQ analysis    — pinneaple_analysis.uq_predict
"""
...
def train_pinn(
    ...
    If ``compiled_losses`` is provided (from pinneaple_physics.compile_physics),
...
def _train_pinn_compiled(model, opt, sched, tc, compiled_losses,
    ...
    """Training loop using losses compiled by pinneaple_physics.compile_physics."""
    for ep in range(1, tc.epochs + 1):
        model.train()
        opt.zero_grad()
        ...
        if ep % log_interval == 0 or ep == 1:
            print(f"  [{name}] epoch {ep:5d}/{tc.epochs}  loss={total.item():.3e}  "
                  f"(pinneaple_physics compiled losses)")
...
def run_uq(
    ...
    """Run UQ analysis using pinneaple_analysis.uq_predict."""
    if not uq_cfg.enabled:
        return None
    try:
        from pinneaple_analysis import uq_predict
    except ImportError:
        warnings.warn("[Arena] pinneaple_analysis not available; skipping UQ.")
        return None
...
def run_inverse(
    ...
    """Run inverse problem using pinneaple_analysis.invert."""
    if not inv_cfg.enabled:
        return None
    try:
        from pinneaple_analysis import invert
    except ImportError:
        warnings.warn("[Arena] pinneaple_analysis not available; skipping inverse problem.")
        return None
...
# ── Dataset loading (pinneaple_data) ──────────────────────────────────────────

def load_pinneaple_dataset(dataset_id: str,
    ...
    """Load a pinneaple_data dataset and split into train/val arrays.
    ...
    """
    try:
        from pinneaple_data import load_dataset
    except ImportError as e:
        raise ImportError("pinneaple_data not available.") from e
